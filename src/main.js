...
import { isTimezoneConfident, resolveTimezoneFromLocation } from './timezone.js';
...
if (!isTimezoneConfident()) {
  console.log('[Timezone] Low confidence — requesting location to resolve timezone...');
  resolveTimezoneFromLocation().then((tz) => {
    if (tz) {
      ...
      console.log('[Timezone] Could not resolve from location. Using UTC offset fallback.');
    }
  });
} else {
